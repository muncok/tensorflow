...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    def fp2fxp(fp_vars, is_vars=True, name='fp2fxp'):
      fxp_v = [ quantemu_ops.quantize_emu(
                ...
                for v in fp_vars ]
      if is_vars:
        return [tf.assign(v, x, name=name)
    ...
    if loss_scale != 1 and fp16_implementation != 'graph_rewrite':
      ...
      grads, vars = zip(*scaled_grad_vars)
      # transform gradients to fxp
      fxp_grads = fp2fxp(grads, is_vars=False)
      # transform trainable_variables to fxp
      fxp_var_assign_op_be = fp2fxp(vars, is_vars=True, name='fp2fxp_be')
      scaled_grad_vars = [(g, v) for g, v in zip(fxp_grads, vars)]
      ...
      # fxp tranform happens befor minimize_op (applying gradients)
      with tf.control_dependencies(fxp_var_assign_op_be):
        minimize_op = optimizer.apply_gradients(unscaled_grad_vars, global_step)
      # transform to fxp after weight updates
      with tf.control_dependencies([minimize_op]):
        vars = tf.compat.v1.trainable_variables()
        fxp_var_assign_op_af= fp2fxp(vars, is_vars=True, name='fp2fxp_af')
      update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
      train_op = tf.group(minimize_op, update_ops,
          fxp_var_assign_op_be, fxp_var_assign_op_af)
    else:
