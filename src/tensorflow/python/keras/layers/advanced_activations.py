...
@keras_export('keras.layers.PACTReLU')
class PACTReLU(Layer):
  """Parametrized Clipping Activation Rectified Linear Unit.

  It follows:
  `f(x) = 0 for x < 0`,
  `f(x) = x for x > 0 and x < alpha`,
  `f(x) = alpha for x >= alpha`,
  where `alpha` is a learned variable per layer.
  ...
  def __init__(self,
               ...
               shared_axes=None,
               **kwargs):
    super(PACTReLU, self).__init__(**kwargs)
    self.supports_masking = True
  ...
  def build(self, input_shape):
    param_shape = list([1]) #list(input_shape[1:])
  ...
  def get_config(self):
    ...
    base_config = super(PACTReLU, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
